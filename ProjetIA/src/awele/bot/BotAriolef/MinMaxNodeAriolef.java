 ...
 */
public abstract class MinMaxNodeAriolef
{
     ...
     */
    public MinMaxNodeAriolef(Board board, int depth, double alpha, double beta)
    {
            ...
            if (board.getPlayerHoles () [i] != 0)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (depth < MinMaxNodeAriolef.maxDepth)
                        {
                            /* On construit le noeud suivant */
                            MinMaxNodeAriolef child = this.getNextNode (copy, depth + 1, alpha, beta);
    ...
    {
        MinMaxNodeAriolef.maxDepth = maxDepth;
        MinMaxNodeAriolef.player = board.getCurrentPlayer ();
    }

    private int diffScore (Board board)
    {
        return board.getScore (MinMaxNodeAriolef.player) - board.getScore (Board.otherPlayer (MinMaxNodeAriolef.player));
    }
     ...
     */
    protected abstract MinMaxNodeAriolef getNextNode (Board board, int depth, double alpha, double beta);
